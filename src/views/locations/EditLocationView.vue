        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white" @click.stop>
          <div class="mt-3 text-center">
            ...
            <div class="mt-2 px-7 py-3">
              <p class="text-sm text-gray-500">Location updated successfully!</p>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    ...
                    <input type="text" v-model="form.contactPersonName"
                      :class="[
                        'appearance-none relative block w-full px-2 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-lg focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500 focus:z-10 sm:text-md',
                        showValidation && !form.contactPersonName.trim() ? 'border-red-500 ring-red-500 focus:ring-red-500 border-2' : ''
                      ]"
...
async function loadLocationData() {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      errorMessage.value = response.message || 'Failed to load location data'
    }
  } catch (err) {
    console.error('Failed to load location data:', err)
    errorMessage.value = 'Network error while loading location data'
  } finally {
    loading.value = false
  }
}
...
async function updateLocation() {
  ...
  try {
    ...
    if (response.success) {
      ...
      errorMessage.value = response.message || 'Failed to update location'
      showErrorModal.value = true
    }
  } catch (err) {
    console.error('Failed to update location:', err)
    errorMessage.value = 'Network error while updating location'
    showErrorModal.value = true
  } finally {
    loading.value = false
  }
